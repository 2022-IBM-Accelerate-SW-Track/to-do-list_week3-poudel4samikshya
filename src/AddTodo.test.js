...
afterEach(() => {
 ...
 test('test that App component renders different colors for past due events', () => {
  ...
  fireEvent.change(inputTask, { target: { value: "Before Due Date"}});
  fireEvent.change(inputDate, { target: { value: newDate}});
  fireEvent.click(element);
  const oldDate = "05/30/2022";
  fireEvent.change(inputTask, { target: { value: "After Due Date"}});
  fireEvent.change(inputDate, { target: { value: oldDate}});
  fireEvent.click(element);
  const colorOld = screen.getByTestId(/Before Due Date/i).style.background;
  const colorNew = screen.getByTestId(/After Due Date/i).style.background;
